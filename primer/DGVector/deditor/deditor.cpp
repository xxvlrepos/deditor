...
int deditor::getIdStatus(QString id){
   if(MapIdStatus.contains(id)) return MapIdStatus[id];
   return 0;
}
...
void deditor::replaceNode(const QModelIndex& ci,const QString& ni){
	...
	int max=schema->getMaxOccursXLS(de);
	if(Ratio==choice && max==1){//вообще очищаем всех детей
		row=0;
